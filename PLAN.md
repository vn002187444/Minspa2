...
- [x] Đăng nhập phân quyền (`ADMIN` hoặc `STAFF`).
- [x] Giao diện Quầy Nhân Viên (`/app/staff`):
...
- [x] Giao diện Quản trị viên (`/app/admin`):
...
- [x] Trình quản trị Blog SEO AI (`/app/admin?tab=blog` hoặc `/app/admin/blog`):
  - [x] Viết bài chuẩn SEO tự động bằng Gemini 3.5 Flash tích hợp nghiên cứu thông tin làm đẹp online thời gian thực.
...
- [x] Chạy nền theo lịch trình thông minh `/api/cron-check` để tự động thực hiện các hành động:
